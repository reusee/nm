...
func TestParse(t *testing.T) {
	nodes, err := Parse(strings.NewReader(testHtml))
	if err != nil {
		t.Fatal(err)
	}
	if len(nodes) != 1 {
...
func TestParseError(t *testing.T) {
	...
	if err := node.Compare(&Node{}); err == nil || !strings.HasPrefix(err.Error(), "number of children") {
...
func TestParseTagMismatched(t *testing.T) {
	nodes, err := ParseBytes([]byte(`<div><p><a></div>`))
	if err != nil {
		t.Fatal(err)
	}
	if len(nodes) != 1 {
